---
title: Multiplayer Flutter
slug: "/workshop/multiplayer-flutter"
authors: ben
---

# 🌶 Warning 🦑

This document is still in progress. Come back on February 28th, 2022. The 2 hour workshop will take place on March 26, 2022. 
Whilst you wait, look at this cat observing my google nest mini.

![Cat observing a Google Nest Mini](winston.jpg)

# Workshop: Building a multiplayer drawing game

Come build a multiplayer drawing game using Flutter, and learn things about Flutter, Firebase, releasing apps, CI/CD, and testing.

## Why read this document

- Prepare your machine and your mind before the day
- Follow along at your own pace
- After the workshop, continue where you left off
- You prefer reading text than watching videos or people

## Features

- Multiple users drawing on the same canvas simultaneously
- End to end: Start, build, ship with CI/CD
- Support for Android, iOS, Web, Windows, macOS and Linux
- Released to production: You can install a more advanced version of the app

## Workshop outline

- Build UI, authentication
- Prepare CI
- Build quiz, missions, timers
- Build drawing canvas
- Send canvas data to other users, and setup realtime capability
- Update canvas with all players drawings

## Pre-requisites

- Development machine 
    - Flutter ready to work. Try to create a template and run the application.

## Workshop walkthrough

TODO

## Conclusion

TODO