...
- Positives:
  ...
  - You can still call Cloudflare workers using Cloudflare Pages Functions's ["Service Bindings"](https://developers.cloudflare.com/pages/platform/functions/bindings/). Your cloudflare pages function can communicate with other workers without incurring network latency. This is because tRPC running on Cloudflare Workers uses the fetch API, which does not go over the network.
- Negatives:
...
- Positives:
  ...
  - You can still use Service Bindings to communicate with other workers, without incurring network latency.
  - Negatives:
...
## Summary 📒
I'm currently using  **tRPC on Cloudflare Pages**. Let me know if you have any questions. I connect to other workers using Cloudflare Pages functions service bindings 🤓
